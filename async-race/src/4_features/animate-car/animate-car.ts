...
export class AnimateCar {
  ...
  static async startAnimate(garageBox: IGarageBox) {
    if(garageBox.startBtn.button.classList.contains('active')) {
      ...
      await this.animate(car, track, response, garageBox.id, garageBox.stopBtn.button, garageBox.startBtn.button);
    }
  }

  static async animate(car: HTMLElement, track: HTMLElement, resonse: IStartStorResponse, id: number, stopBtn: HTMLElement, startBtn: HTMLElement) {
    ...
    function step(timeStamp: number) {
      ...
      if (!done && stopBtn.classList.contains('active')) {
        requestAnimationFrame(step);
      }
    }
  }
...
/* export async function animate(car: HTMLElement, track: HTMLElement, resonse: IStartStorResponse, id: number, stopBtn: HTMLElement, startBtn: HTMLElement) {
