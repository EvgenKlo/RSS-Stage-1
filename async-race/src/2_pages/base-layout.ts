...
export class BaseLayout {
  ...
  private mainValie = this.garageView.buildGarage();
  ...
  private createButtonToHeader(parentElement: HTMLElement) {
    ...
    const buttonsClasses = ['header-button_to-garage', 'header-button_to-winners'];
    buttons.forEach((item, index) => {
      const button = createElementInDOM('div', ['header-button', buttonsClasses[index]], parentElement);
